...
class MovingShift:
    ...
    def __init__(self, max_shift_x=0, max_shift_y=0, filter_thr_px=1):
        ...
        self.shift_x = -1*max_shift_x
        self.shift_y = -1*max_shift_y
    ...
    def __call__(self, results):
        ...
        random_shift_x = self.shift_x
        random_shift_y = self.shift_y
        new_x = max(0, random_shift_x)
        ori_x = max(0, -random_shift_x)
        new_y = max(0, random_shift_y)
        ori_y = max(0, -random_shift_y)

        if (self.max_shift_x != 0):
            self.shift_x += 1
            assert self.shift_x < self.max_shift_x
        if (self.max_shift_y != 0):
            self.shift_y += 1
            assert self.shift_y < self.max_shift_y

        # with open("work_dirs/vizualize/log.txt", mode='a') as f:
        #     f.write('img_shape:{},{}\n'.format(img_shape[0], img_shape[1]))
        #     f.write('move:{},{}\n'.format(self.shift_x, self.shift_y))
        ...
        for key in results.get('bbox_fields', []):
            ...
            #         f.write('gt_bboxes:')
            #         for bbox in bboxes:
            #             f.write('{:.5f},{:.5f},{:.5f},{:.5f};'.format(bbox[0],bbox[1],bbox[2],bbox[3]))
            #         f.write('\n')
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(max_shift_x={self.max_shift_x}, '
        return repr_str
...
class MovingShift1:
    ...
    def __call__(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            # clip border
            bboxes[..., 0::2] = np.clip(bboxes[..., 0::2], 0, img_shape[1]+self.max_shift_x)
            bboxes[..., 1::2] = np.clip(bboxes[..., 1::2], 0, img_shape[0]+self.max_shift_y)
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_h, img_w = img.shape[:2]
            new_img = np.zeros((img_h+self.max_shift_y, img_w+self.max_shift_x, 3), img.dtype)
            new_img[new_y:new_y + img_h, new_x:new_x + img_w] \
                = img
...
class MovingRandomShift:
    ...
    def __call__(self, results):
        ...
        random_shift_x = random.randint(-self.max_shift_x,
                                        self.max_shift_x)
        random_shift_y = random.randint(-self.max_shift_y,
                                        self.max_shift_y)
        new_x = max(0, random_shift_x)
        ori_x = max(0, -random_shift_x)
        new_y = max(0, random_shift_y)
...
class SolidShift:
    ...
    def __init__(self, shift_x=0, shift_y=0, filter_thr_px=1):
    ...
    def __call__(self, results):
        ...
        new_x = max(0, self.shift_x)
        new_y = max(0, self.shift_y)

        # TODO: support mask and semantic segmentation maps.
        for key in results.get('bbox_fields', []):
            bboxes = results[key].copy()
            bboxes[..., 0::2] += self.shift_x
            bboxes[..., 1::2] += self.shift_y
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_h, img_w = img.shape[:2]
            new_img = np.zeros((img_h+0, img_w+256, 3), img.dtype)
            new_img[new_y:new_y + img_h, new_x:new_x + img_w] \
                = img
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(shift_x={self.shift_x}, '
        return repr_str

@PIPELINES.register_module()
class GADA:
    def __init__(self, ratio=0.5, filter_thr_px=1):
        self.shift_x = 0
        self.shift_y = 0
        self.filter_thr_px = int(filter_thr_px)
        self.ratio=ratio
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Shift results.
        """
        if (np.random.random_sample() < self.ratio):
            return results
        ...
        random_shift_x = random.randint(-4, 4)
        random_shift_y = random.randint(-4, 4)
        target_box = random.randint(0,len(bboxes))
        left, top, right, bottom = bboxes[target_box]

        grid_size = 8
        strides = [8,16,32,64,128]
        regress_ranges = [[-1,64], [64,128], [128,256], [256,512], [512, 10e5]]
        for stride, regress_range in zip(strides, regress_ranges):
            cx = float(((left+right)/2)//stride)*stride + stride/2
            cy = float(((top+bottom)/2)//stride)*stride + stride/2
            length = [cx - left, cy - top, right - cx, bottom - cy] 
            bbox_size = max(length)
            if (regress_range[0] <= bbox_size < regress_range[1]):
                grid_size = stride

        self.shift_x = 0
        for x in range(grid_size):
            if (int((left+x+right+x)/2)%grid_size ==0):
                self.shift_x = x

        self.shift_y = 0
        for y in range(grid_size):
            if (int((top+y+bottom+y)/2)%grid_size ==0):
                self.shift_y = y

        self.shift_x += random_shift_x
        self.shift_y += random_shift_y
        self.shift_x = np.clip(self.shift_x, 0, 128)
        self.shift_y = np.clip(self.shift_y, 0, 128)
        new_x = max(0, self.shift_x)
        new_y = max(0, self.shift_y)
        ...
        for key in results.get('bbox_fields', []):
            ...
            # clip border
            bboxes[..., 0::2] = np.clip(bboxes[..., 0::2], 0, img_shape[1]+128)
            bboxes[..., 1::2] = np.clip(bboxes[..., 1::2], 0, img_shape[0]+128)
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_h, img_w = img.shape[:2]
            new_img = np.zeros((img_h+128, img_w+128, 3), img.dtype)
            new_img[new_y:new_y + img_h, new_x:new_x + img_w] \
                = img
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(shift_x={self.shift_x}, '
        repr_str += f'(shift_y={self.shift_y}, '
        return repr_str
...
class GADA_YOLOX:
    ...
    def __call__(self, results):
        ...
        grid_size = 8
        strides = [8,16,32]
        regress_ranges = [[-1,64], [64,128], [128, 10e5]]
        for stride, regress_range in zip(strides, regress_ranges):
            cx = float(((left+right)/2)//stride)*stride + stride/2
            cy = float(((top+bottom)/2)//stride)*stride + stride/2
            length = [cx - left, cy - top, right - cx, bottom - cy] 
        ...
        self.shift_x += random_shift_x
        self.shift_y += random_shift_y
        self.shift_x = np.clip(self.shift_x, 0, 32)
        self.shift_y = np.clip(self.shift_y, 0, 32)
        new_x = max(0, self.shift_x)
        new_y = max(0, self.shift_y)
        ...
        for key in results.get('bbox_fields', []):
            ...
            # clip border
            bboxes[..., 0::2] = np.clip(bboxes[..., 0::2], 0, img_shape[1]+32)
            bboxes[..., 1::2] = np.clip(bboxes[..., 1::2], 0, img_shape[0]+32)
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_h, img_w = img.shape[:2]
            new_img = np.zeros((img_h+32, img_w+32, 3), img.dtype)
            new_img[new_y:new_y + img_h, new_x:new_x + img_w] \
                = img
